...
void insertLeft(struct LinkList** ll, int data){
	struct LinkList* newNode = (LinkList*)calloc(1, sizeof(struct LinkList));
...
struct LinkList* multiple(struct LinkList* a, struct LinkList* b){
	int sizeA = size(a);
	int sizeB = size(b);
	struct LinkList* result = (LinkList*)calloc(1, sizeof(struct LinkList));
...
int main(){
	char* data = (char*)calloc(0100, sizeof(char));
